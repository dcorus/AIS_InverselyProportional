...
\title{Effective Inversely Proportional Hypermutations for Unimodal and Multimodal Optimisation\\ - Supplementary Material: Self Contained Paper -}
...
The 15-page version of this article is available at:xxx.
}
...
Clonal selection algorithms are also \rev{popular} in multi-objective optimisation~\cite{CoelloEMO2025,Coello2022,COY25,HuangZhou2020,EMOReview}. 
...
\new{Two of the most popular} clonal selection algorithms, Clonalg \cite{DecastroVonzuben2002}   and \new{the} Opt-IA \new{immune algorithm~\cite{CutelloNicosiaPavoneTimmis2007} (as well as their many variants~\cite{ClonalSelectionReview}) both employ IPHs}.  
...
all supporting lemmata.  The argument proceeds in two 
...
\textcolor{black}{Each \emph{ageing epoch} begins with a uniformly random reinitialisation; by Lemma~\ref{lem:junction-first-density-Bk} it
...
By Lemma~\ref{lem:junction-first-density-Bk} we reach the junction $J$ with probability \ \(1-o(1)\).
	\rerev{Given that an epoch reaches the junction}, \rerev{Lemma \ref{lem:BJO-epoch}  and Lemma \ref{lem:DPO-epoch} respectively} give
...
Multiplying by the cost of each epoch, \rerev{which is $\Theta(n^3+n\,\tau)$ since we climb a LeadingOnes slope and then spend $\tau$ iterations on the junction,} we get,
	\begin{align*}
		\EE[\text{evals}]
		&\le
