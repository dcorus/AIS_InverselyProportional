...
\newcommand{\rerev}[1]{\textcolor{black}{#1}}
...
%        
%}
\author{Dogan Corus	 and Pietro S. Oliveto,~\IEEEmembership{Senior member,~IEEE,}
%	% <-this % stops a space
	%\author{Anonymous authors	
	% <-this % stops a space

\thanks{
Dogan Corus is with the Department of Computer Engineering, Istanbul Bilgi University, Istanbul, Turkey. 
Pietro S. Oliveto is with the Department of Computer Science and Engineering, Southern University of Science and Technology (SUSTech), Shenzhen, China
(emails: dogan.corus@bilgi.edu.tr, olivetop@sustech.edu.cn). 

An appendix is available at: xxx. A self-contained version of this article is available at:yyy.
}
...
In this paper, we examine %the more commonly used dynamic 
inversely proportional  hypermutations (IPH) with mutation potential for which previous theoretical analyses have shown disappointing performance. We first show that considerable speed-ups
in hillclimbing phases may be achieved compared to static hypermutations in the ideal situation where the optimum is known. % if the mutation rates decrease appropriately with either the fitness or the distance to the optimum. 
Afterwards, we define a simple (1+1)~Opt-IA that uses IPH and ageing for realistic applications where optimal solutions are unknown. The aim of this AIS is to approximate the ideal behaviour of the IPH better and better as the search space is explored and local optima of higher quality are identified.
...
Three recent hot topics in the theory of randomised search heuristics for optimisation in general and in artificial intelligence in particular have been the benefits of mutation operators with higher mutation rates than traditionally employed \peter{\cite{FQWGECCO18,FGQWPPSN18,CorusOlivetoYazdani2021TEVC,DoerrGECCO2017FastGA,OX2026}} (or standard operators using higher parameter values than those previously recommended \peter{\cite{CorusOlivetoTEVCsteadyGA2017,JumpTEVC2017,CorusOlivetoGecco2019,OlivetoSudholtWitt2020,DoerrDoerrEbel2015,LenglerTEVC2019}}), the automatic adaptation of the mutation rate \peter{ \cite{DoerrDoerrBookChapter,DoerrEtAl2016B,LissovoiEtAl2020ECJ,LOWAAAI2020,DOW2026,OWWXPPSN26,OWPPSN26,DOW2026,MaEtAl2025}} and the analysis of when non-elitist search heuristics are beneficial compared to elitist ones \cite{OlivetoPaizJorgeSudholt2017Algorithmica,LOW2023,Lehr2021AAAI,DoerrComma2020,CaseLehreTEVC,DangJansenLehreGECCO2015,DangLehreFOGA2015,LenglerSchillerSieberling2024,HeviaSudholtAlgo24}. 
...
Clonal selection algorithms are also \rev{popular} in 
multi-objective optimisation~\cite{CoelloEMO2025,Coello2022,COY25,HuangZhou2020,EMOReview}. 
...
antigen (i.e., global optima) during affinity maturation. Often such operators are referred to as {\it inversely proportional hypermutations} (IPHs). 
\new{Two of the most popular} clonal selection algorithms, Clonalg \cite{DecastroVonzuben2002}   and \new{the} Opt-IA \new{immune algorithm~\cite{CutelloNicosiaPavoneTimmis2007} (as well as their many variants~\cite{ClonalSelectionReview}) both employ IPHs. } % inversely proportional hypermutations}.  
...
As a result, in practical applications information about the problem is used to \new{find} bounds on (or estimates of)  the value of the global optimum\rerev{.}\footnote{Alternatively, the fitness of the best candidate solution is sometimes used and 
...
Hence, the behaviour of the mutation operator leads to very inefficient performance even for the simple \textsc{OneMax} function when the optimum is assumed to be known. 
...
of being slower at hillclimbing during the exploitation phases \rerev{\textemdash} it is up to a linear factor slower than the standard bit mutation (SBM) used by evolutionary algorithms for easy unimodal functions such as \textsc{OneMax} and \textsc{LeadingOnes}.
...
A previous runtime analysis for \textsc{OneMax} of the inversely proportional hypermutations with mutation potential used by Opt-IA~\cite{JansenZarges2011} has shown that the mutation rate is always in the range $[(c/2)n, cn]$ where $M=cn$ is the highest mutation potential. Hence it does not decrease proportionally with the decrease in distance to the optimum as desired and, as a result, no asymptotic speed-ups are achieved compared to static mutation potentials. 
...
\begin{table*}[t!]
	...
	$\beta:= \log_2 \frac{20}{c}$\textcolor{black}{, where $c\le 1$ is the constant factor in the \linHD\ potential $M = \lfloor c\cdot H(x,\mathit{best})\rfloor$}.
	}
\begin{center}
	\resizebox{2\columnwidth}{!}{
		...
		\begin{tabular}{ |l l l l l| }
			...
			Opt-IA$_{\textsc{static}}$ & $\Theta(n^2\log n)$ \cite{CorusOlivetoYazdani2019TCS} & $\Theta(n^3)$\cite{CorusOlivetoYazdani2019TCS} &$O(\frac{n^{d+1}\cdot e^d}{d^d})$ \cite{CorusOlivetoYazdani2019TCS} & \rerev{\color{black}{$O(n\tau + n^{2k+3} + n^{\beta+1} + n^3 + n^{2k+5}/\tau + n^{\beta+3}/\tau)^*$}}
			\T\B\\
			...
			Opt-IA$_{\textsc{FirstBest}}$ & $\Theta(n \log n)$ & $\Theta(n^3)$ &$O\left(\frac{n^{5}\log(n/d)}{d^{3}} + n^{2}\log d\right)$&    \rerev{$O(n\tau + n^{2k+3} + n^{\beta+1} + n^3 + n^{2k+5}/\tau + n^{\beta+3}/\tau)$}
			\T\B \\
...
\rerev{The idea behind inversely proportional hypermutation operators is to decrease the mutation potential as the optimum is approached rather than maintaining a fixed static potential throughout the run.}

The original inversely proportional hypermutation operator (\IPHfcm{}) proposed for Opt-IA uses mutation potential  $M=\lceil(1-f_{\text{OPT}}/v)cn\rceil $ for minimisation problems, where \rerev{$f_{\text{OPT}} > 0$} is the optimal fitness value, $v>0$ is the fitness of the current individual, 
...
In an analysis for \rev{$\textsc{ZeroMin}:=n+1-\textsc {OneMax}$}~\cite{JansenZarges2011}, such a mutation potential was shown not to decrease below $cn/2$, with $cn$ being the highest possible mutation potential.
 ...
 \rerev{In particular, it is well understood that fitness-proportional selection is sensitive to the difference between the fitness values of candidate solutions, i.e., it struggles to distinguish between solutions that have similar, yet different, fitness values~\cite{OlivetoWittTCS2014,OlivetoWitt2015,Whitley1989,NeumannOlivetoWitt2009}. For this reason, nowadays selection operators that rank individuals by fitness (e.g., tournament, linear ranking, comma selection) are generally preferred. We also consider a  third operator using a Hamming distance measure  that  avoids using the fitness values directly for the exponential decrease of the mutation rate because the ideal amount of bits to be flipped clearly depends on the genotypic distance of the candidate solution to the optimum rather than on their difference in fitness values.}
...
\end{lemma}
\begin{proof}
	$n^{\frac{1}{n^\epsilon}} = e^{\frac{\ln(n)}{n^\epsilon}} = 1 + \frac{\ln(n)}{n^\epsilon} + o(\frac{\ln(n)}{n^\epsilon})$ by the Taylor series of the exponent.
\end{proof}



\begin{table}[t!]
\caption{Comparison of the expected runtimes obtained by different hypermutation schemes \rerev{in ideal conditions} for \textsc{OneMax} and \textsc{LeadingOnes}. The original IPH operator of Opt-IA \cite{CutelloNicosiaPavoneTimmis2007}  has the same expected runtimes as the $(1+1)~$IA using static hypermutations \cite{JansenZarges2011}. \new{`$^*$' indicates that the result can be easily drawn from the analysis for \textsc{OneMax} \cite{Zarges2008}.}}
\begin{center}
\resizebox{\columnwidth}{!}{
	\begin{tabular}{ |l l l| }
		...
		\T\B\\
		FCM with \expoF & \textcolor{black}{$\Theta(n^{3/2}/\log n)$} & \textcolor{black}{$\Theta(n^3/\log^2 n)$}
		\T\B \\
		FCM with \expoHD & \textcolor{black}{$\Theta(n^{3/2}/\log n)$} & \textcolor{black}{$\Theta(n^{5/2}/\log^2 n)$}
		\T\B \\
...
\begin{figure}[t!]
\centering
\includegraphics[width=0.30\textwidth]{a}
...
Notice that for \textsc{OneMax} the behaviours of   {\expoF } and {\expoHD }  are the same\peter{, while the static hypermutation uses potential $n$ (i.e., $c=1$).} 
...
an improvement is at most $1-(p-q)/(p+q)=1-(n-2i)/n=\frac{2i}{n}$ by the Ballot theorem, where $i=H(x, opt)$. This means that we need to wait at least $\frac{n}{2i}$ iterations to see an improvement and each time the mutation operator fails to improve the fitness {(\color{black}$\frac{n}{2i} - 1$ times in expectation)}, $i$ fitness function evaluations will be wasted. The expected time to optimise \textsc{OneMax} is larger than \textcolor{black}{$(1-2^{-\Omega(n)})\sum_{i=1}^{n/3} {\color{black}\left(\frac{n}{2i}-1\right)} \cdot i = \Omega(n^2)$} \rerev{where at least $n/3$ improvements are required since FCM allows the operator to improve the \textsc{OneMax} value by at most one per application. The factor $(1-2^{-\Omega(n)})$ accounts for the Chernoff bound on the initial number of $0$-bits, which holds w.o.p.}.
\end{proof}
...
\begin{figure}[t!]
\centering
\includegraphics[width=0.30\textwidth]{b}
...
We conclude} that {\expoHD } provides larger hill-climbing \rerev{(i.e., imitating local search)} speed-ups compared to the other mutation potentials and is stable to the scaling of fitness functions.
...
In this section, we consider the usage of IPH in realistic applications where the optimum is unknown.
 ...
 We combine \IPHfcm{} using {\linHD } with hybrid ageing, as in the Opt-IA AIS \cite{CutelloNicosiaPavoneTimmis2007}, and embed it in a $(1+1)$~Opt-IA shown in Algorithms \ref{alg:alg1+ageing} and \ref{alg:alg2+ageing}. \new{To use \rev{an} ageing operator, an age $\alpha(x)$, is assigned to the individual $x$.} After the Hypermutate$(x)$ operator, the ageing operator is applied.
 	Among the different variants of ageing, {\it hybrid} ageing has been shown to be very efficient at escaping local optima~\cite{OlivetoSudholt2014ageing,CorusOlivetoYazdani2019TCS,CorusOlivetoYazdaniFASTAIS2018,CorusOlivetoYazdaniPPSN2018Approx}. Using this operator,  individuals are assigned with \rerev{an initial age of zero}. During each iteration of the algorithm, the age  increases by 1 and is passed to the offspring if it does not improve over its parent's fitness. If the offspring is fitter than the parent, then its age is set to 0. At the end of each iteration, any individual with age larger than a threshold $\tau$ is removed with probability \rev{$p_{\text{die}}=1/2$}. In case there is no other individual left, a new individual is initialised uniformly at random\footnote{When a population is used, the required number of individuals are re-initialised to reach the population size again after ageing is applied.}. \rerev{Throughout the paper we say that ageing is {\it triggered} to refer to the event that the age of an individual has crossed the threshold $\tau$, the individual is removed from the population and is re-initialised}.
 
 Ageing has been shown to enable algorithms to escape from local optima either by identifying a gradient leading away from it or by restarting the whole optimisation process. 
 \rerev{Thus, the ageing parameter $\tau$ should be set to the time the user wishes to wait for a mutation to escape a local optima before giving up and recurring to other means (i.e., restarts or non-elitism). Nevertheless, the parameter should be set higher than the time required to confidently believe that the individual is trapped on a local optima in the first place.} 
 Our aim behind the algorithm design is that by escaping local optima via ageing, the number of local optima that are identified by the algorithm increases over time. Thus as time goes by the IPH should approximate its ideal behaviour better and better.
 
\textcolor{black}{Concerning the IPH operator for practical applications we  reintroduce the parameter \rerev{$0 < c \leq 1$} for determining the mutation potential when it grows linearly with the Hamming distance as in the original Opt-IA~\cite{JansenZarges2011}:}  
 \textcolor{black}{
...
\rerev{So, the operator can be considered as a compromise between always updating the best seen and never updating the best seen when a new solution of equal fitness to the current best is encountered.}  
...
\FOR{$w \in \{x,y\}$}
 	            ...
 	            \textcolor{black}{\STATE \hspace{\algorithmicindent}
...
single-bit flips \rerev{and accept offspring of equal or greater fitness} thus, behave identically to \rerev{Random Local Search (RLS) which always flips exactly one bit}.
...
\begin{figure}
\centering
\includegraphics[width=0.30\textwidth]{cliff_updated}
\caption{\textcolor{black}{The \textsc{Cliff}$_d$ function for $n=50$, $d=10$.
...
\begin{theorem}\label{th:expoHD-cliff}
	The \textcolor{black}{\firstBest~with} \linHD\ (factor $c<\tfrac{1+4\varepsilon}{3-4\varepsilon}$) and ageing parameter $\tau=\Omega(n^{1+\rerev{\varepsilon'}})$
	optimises \cliff{} with $d<n(\tfrac{1}{4}-\rerev{\varepsilon})$ \rerev{for any arbitrary small constants $\varepsilon,\varepsilon'>0$} in expected
	\[
...
algorithm into a
\textcolor{black}{canonical
starting configuration on the second slope.}

\rerev{We call each individual hypermutation application an \emph{attempt}.
A \emph{phase} groups all consecutive attempts at a fixed zero-bit count~$z$;
it begins when a successful improvement leaves $z$ zero-bits and ends when
either the next improvement (reducing to $z-1$ zeros) or a
 return to the cliff-top occur.
A phase ending with an improvement is called \emph{successful}.
Since each successful phase reduces the zero-count by exactly one, we
index them by $r = r_0, r_0+1, \dots, d$, where $r$ counts the total
number of fitness improvements completed from the cliff bottom.}

\textcolor{black}{Throughout the \cliff{} analysis we write $\mathrm{HD}(r)$ for
...
\begin{proof}[Proof [Theorem~\ref{th:expoHD-cliff}]]
	...
	\[
	r_0=2,\qquad \text{H}(r_0)=2
	\]
	with probability
	\[
	p_{\mathrm{can}}(n,d)\ \ge\ \rerev{\frac{3}{128}}\Big(\frac{d}{n}\Big)^{\!3}.
	\]
	...
	\smallskip
	Under the reference-update rule the start on the second slope is $r_0=2$, $\text{H}(r_0)=2$, so $s_0=0$ and with $\theta=\tfrac1c-1$ define
	\[
	...
	\[
	\textcolor{black}{\EE\!\big[e^{\lambda Y_z}\big]\ \le\ \frac{1-\rho}{\,1-\rho e^{2\lambda}\,} \;\eqqcolon\; K(\lambda).}
	\]
	...
	\rerev{Applying Corollary~\ref{cor:ville-exponential} to the re-indexed process $(B_{r_0+t})_{t\ge0}$ with $b=B_{r_0}=2\theta$} yields
	\[
	...
	\]
	iterations, which is $o(\tau)$ since $\tau=\Omega(n^{1+\rerev{\varepsilon'}})$. \textcolor{black}{Since the mutation potential satisfies $M \le c\,n$ (where $c$ is the constant
...
\textcolor{black}{
\begin{corollary}
	The \textcolor{black}{\lastBest~with} \linHD\ \new{(factor $c<\tfrac{1+4\varepsilon}{3-4\varepsilon}$)} and ageing parameter $\tau=\Omega(n^{1+\rerev{\varepsilon'}})$
	optimises \cliff{} with $d<n(\tfrac{1}{4}-\rerev{\varepsilon})$ \rerev{for any arbitrary small constants $\varepsilon,\varepsilon'>0$} in expected
	\[
...
\end{cases}
\]
\textcolor{black}{Here $x_{i>r}$ denotes the suffix of $x$ at positions $i > r$, i.e.,
$x_{i>r} := (x_{r+1}, x_{r+2}, \dots, x_n)$.}
 \textcolor{black}{Note that  $r := 2k+1$ is an odd number, as required for the block-jump prefix $0^r1^{(0.9n-r)}*$
...
\begin{figure}[t]
\centering
\includegraphics[width=0.40\textwidth]{ltj_diagram}
\caption{\textcolor{black}{The \textsc{LeadingTrapJump}$_{k,\rho}$ landscape (schematic,
$n=30$, $k=1$, $\rho=0.85$).
...
The landscape consists of a long \textsc{LeadingOnes} slope rising from fitness 
...
\textcolor{black}{$0^{r}1^{(0.9n-r)}$}; reaching 
this prefix is therefore equivalent to entering the leading ones path to the local optimum \textcolor{black}{
$0^{r}1^{(0.9n-r)}$}.}  
...
\textcolor{black}{Both structures are hidden behind the penalised barrier (illustrated in
...
the \textsc{LeadingOnes} slope back to $J$ before any off-slope structure is encountered.  
...
The  $(1+1)$~Opt-IA$_{\textsc{FirstBest}}$~with ageing threshold $\tau=\Omega(n^{1+\epsilon})$ using  \linHD~with $c<5/8$  identifies both local and global optima of  $\mathrm{LTJ}_{k,\rho}$ in
	\rerev{$O(n\tau + n^{2k+3} + n^{\beta+1} + n^3 + n^{2k+5}/\tau + n^{\beta+3}/\tau)$} fitness function evaluations	where $\beta := \log_2 \frac{20}{c}$.
\end{theorem}
...
Second, an epoch \rerev{that reaches} $J$ performs sufficiently long jumps to reach the beginning of the block-jump slope with probability $\Theta(n^{-(2k+2)})$ per iteration.
Third, an epoch \rerev{that reaches} $J$ performs a logarithmic jump onto the deep-prefix plateau with probability $\Theta(n^{-\beta})$ per iteration.
...
Within $n^2$ iterations, the algorithm performs at least $0.9n$ \textsc{LeadingOnes} 
...
\rerev{A similar argument shows that $\mathcal{B}_k$ is not entered before $J$.
Recall that $\mathcal{B}_k = \{0^r 1^{(0.9n-r)}*\}$ with $r = 2k+1$.
If $\mathrm{LO}(x)$ is not yet close to $0.9n$, the suffix contains
$\Omega(n)$ unbiased bits, so $\Omega(n)$ specified zero-bits must be
flipped to ones simultaneously, an event with probability
$e^{-\Theta(n)}$ per iteration.
If instead $\mathrm{LO}(x) \geq (0.9-\delta)n$ for a constant $\delta > 0$,
then $\Omega(n)$ leading-one positions must survive the hypermutation
for FCM to accept; entering $\mathcal{B}_k$ requires the $r+1$ target
positions to be flipped before any of these $\Omega(n)$ protected
positions, which happens with probability $O(n^{-(r+1)}) = O(n^{-(2k+2)})$.
A union bound over $n^2$ iterations gives $o(1)$ in both cases.}
Thus the slope is climbed to $J$ with high probability.



\textcolor{black}{The following two lemmata will condition throughout on the event that during an ageing epoch the junction $J$ is reached.
...
Thus it suffices to flip the prefix to $0^L *$ \rerev{while retaining at least $\rho n$ ones}.
Under the same high-Hamming-distance event ensuring $M(x)\ge cn/20$, \rerev{we condition on all $L$ prefix positions appearing among the first $\lfloor cn/20\rfloor$ flips; this happens with probability at least $(c/20)^L = n^{-\beta}$, and at most $cn/20 < n/32$ ones are lost in the process, preserving the density requirement.}
Evaluating over $\tau$ iterations gives the lemma.



\textcolor{black}{Each \emph{ageing epoch} begins with a uniformly random reinitialisation and by Lemma~\ref{lem:junction-first-density-Bk} it
...
\rerev{The ascent to $J$ takes $\Theta(n^2)$ iterations, each evaluating $\Theta(n)$ individuals (since the reference remains at $J$ from the previous epoch, giving $M=\Theta(n)$), contributing $\Theta(n^3)$ evaluations.  Once stuck at $J$, the epoch runs for $\tau$ iterations each evaluating $\Theta(n)$ individuals, contributing $\Theta(n\tau)$ evaluations.  The total cost per epoch is therefore $\Theta(n^3 + n\tau)$.}
...
\textcolor{black}{Since $B$ (entering $\mathcal{B}_k$) and $D$ (entering $\mathcal{D}_\rho$) are
...
\begin{proof}[Proof [Theorem~\ref{thm:main-tau-final}]]
	...
	\rerev{Given that an epoch reaches the junction}, %\(J\) (prob.\ \(1-o(1)\)), 
	\rerev{Lemma \ref{lem:BJO-epoch}  and Lemma \ref{lem:DPO-epoch} respectively} give
	%\[
...
In particular the \rev{$(1+1)$~Opt-IA$_{\textsc{FirstBest}}$} can escape local optima in three different ways: by mutating into far away solutions (see \textsc{LeadingTrapJump}), by accepting solutions of inferior quality (see \textsc{Cliff}) and naturally, \rerev{as understood regarding ageing for a long time,} by automatically reinitialising the trapped solutions via ageing~\cite{HorobaJansenZarges09,JansenZarges2011differentageing,JansenZarges2010restart,OlivetoSudholt2014ageing}.
%In particular, 
Compared to the conference version of this paper, we have now proposed an Opt-IA algorithm with IPH that has the capability to escape from local optima via large mutations, something that was not possible with the previous version since the mutation rate remained lowest on the local optima.
...
The authors thank Donya Yazdani for her contribution to the conference version of this article.
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{dcorus}}]
{Dogan Corus}
received the B.Sc. and M.Sc degree in industrial engineering from Koç University, İstanbul, Turkey, and the Ph.D. degree in computer science from the University of Nottingham, Nottingham, U.K. He is Assistant Professor of computer engineering at İstanbul Bilgi University, İstanbul, Turkey. His current research interests include the runtime analysis of genetic  algorithms and artificial immune systems.
\end{IEEEbiography}
%
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{Oliveto2}}]
{Pietro S. Oliveto}
 received the Laurea degree in computer science from the University of Catania,
Catania, Italy, in 2005, and the Ph.D. degree from the University of Birmingham, Birmingham, U.K. in 2009.
He is Professor of computer science and Head of the Theory of AI Lab at the Southern University of Science and Technology (SUSTech),
Shenzhen, China. His current research interests include the
performance analysis of bio-inspired computation techniques, covering evolutionary algorithms,
genetic programming, artificial immune systems, algorithm configuration and hyper-heuristics.
Prof. Oliveto has been an EPSRC Ph.D.$^+$ Fellow and an EPSRC
Post-Doctoral Fellow at Birmingham and Vice-Chancellor's Fellow and EPSRC Early Career Fellow at Sheffield.
%Use $\backslash${\tt{begin\{IEEEbiography\}}} and then for the 1st argument use $\backslash${\tt{includegraphics}} to declare and link the author photo.
%Use the author name as the 3rd argument followed by the biography text.
\end{IEEEbiography}
%
