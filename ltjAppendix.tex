...
\begin{proof}
	...
	Before $J$, $\mathcal{B}_k$ or $\mathcal{D}_\rho$ is hit, the first flip equals the leftmost zero with probability $1/n$ and is accepted
	...
	\]
	for some constant $a>0$. Hence $J$ is reached within $T=n^2$ iterations with probability $1-e^{-a n}$ if we condition on that neither $\mathcal{B}_k$ nor $\mathcal{D}_\rho$ is sampled first.
	
	\medskip\noindent
	\textbf{(B) $\mathcal{D}_\rho$ before $J$ is $o(1)$.}
	Let $\alpha:=|x|_1/n$ and \textcolor{purple}{$\alpha^\top:=|x^\top|_1/n$.}
