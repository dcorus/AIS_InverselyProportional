...
\begin{definition}[Notation, algorithm, and solution types]
	...
	\begin{itemize}
		...
		$
		\item \emph{Deep‐Prefix Optima (DPO):} for $\rho\in(1/2,0.9)$,
		$\displaystyle
		\mathcal{D}_\rho
...
\begin{lemma}[Junction precedes DPO and $\mathcal{B}_k$ with probability $1-o(1)$]
	...
	Choose any $\rho\in(1/2,0.9)$ and any $\xi\in(0,\rho-\tfrac12]$ such that
	\[
...
{\color{red}SSSSSSSSSSSSSSSSSSSSS}
\begin{lemma}[Per-epoch probability of accepting the block-jump optimum]
	...
	then the probability that the epoch accepts the unique block-jump optimum
	
	\[
	y^{\star}=0^{\,2k+1}\,1^{\,n-2k-1}
	\]
...
\begin{proof}
	...
	\medskip\noindent
	\emph{Per-iteration success probability.}  
	Starting from a parent in \(J\), obtaining \(y^{\star}\) within one
	hypermutation requires flipping exactly the \(r\)
	leading positions \(\{1,\dots,r\}\) (changing \(1\!\to\!0\))
	\emph{and} flipping the single zero at position \(0.9n\)
	(changing \(0\!\to\!1\))
	before FCM stops; no other flips matter.
	Sampling without replacement,
	\[
	p_{\text{iter}}
	\;=\;
	\frac{(M)_{\,r+1}}{(n)_{\,r+1}}
	\;\ge\;
	\bigl(M/n\bigr)^{\,r+1}
	\;\ge\;
	(1/60)^{\,r+1}
...
\begin{lemma}[Per-epoch probability of accepting a DPO]
	\label{lem:DPO-epoch}
	Fix the IPH constant \(c=\tfrac13\) and any density parameter \(\rho\in(1/2,0.9)\).  
	...
	\tau\,n^{-\,\beta}\bigr\}\Bigr),
	\]
	where \(\beta=\log_{2}60<6\).  The implicit constant depends only on the fixed
	parameters \(c\) and \(\rho\).
\end{lemma}
\begin{proof}[Proof of Lemma \ref{lem:DPO-epoch} (revised)]
	...
	epoch (also in $J$), and the new parent $x$ itself lies in~$J$.
	Hence
	\[
...
\begin{proof}
	...
	\bigr).
	\]
	All hidden constants depend only on the fixed parameters \(k\) and \(c\).
\end{proof}
