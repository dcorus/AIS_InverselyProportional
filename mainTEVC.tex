...
\begin{lemma}[Junction precedes DPO and BJO with probability $1-o(1)$]
	\label{lem:junction-first}
	Fix $c\in(0,\tfrac13)$ and $k\ge 1$. Consider the $(1{+}1)$ Opt-IA with IPH and FCM($\ge$),
	mutation potential $M(x)=\max\{1,\lfloor c\cdot \mathrm{HD}(x,x^\top)\rfloor\}$, and an arbitrary fixed
	reference $x^\top$ (possibly the current parent, a local optimum, a DPO, or a BJO).
	Let $J:=\{x:\mathrm{LO}(x)=(9/10)n-1\}$. The process starts from a uniformly random $x^{(0)}$.
	Let $\mathcal{E}_{\mathrm{junction}}$ denote the event that the algorithm reaches $J$
	before sampling either a deep-prefix optimum (DPO) $0^{\lfloor\log n\rfloor}y$ with $|y|_1>\tfrac{3n}{4}$
	or the block-jump optimum (BJO) $y^\star:=0^{2k+1}1^{n-2k-1}$. Then
	\[
	\Pr(\mathcal{E}_{\mathrm{junction}})=1-o(1).
	\]
\end{lemma}

\begin{proof}
	Write $L:=\lfloor\log n\rfloor$ and $T:=n^2$.
	We show that, up to $T$ iterations: (A) we reach $J$ w.h.p., (B) the probability of creating a DPO
	before $J$ is $o(1)$, and (C) the probability of creating the BJO before $J$ is $e^{-\Omega(n)}$.
	Combining gives the claim.
	
	\medskip\noindent
	\emph{(A) Reaching $J$ within $T=n^2$ iterations w.h.p.} In each iteration prior to $J$, the first flip
	hits the current leftmost zero with probability exactly $1/n$ and yields an immediate level increase
	by FCM($\ge$). Let $X\sim\mathrm{Bin}(T,1/n)$ count such increases; $\mathbb{E}[X]=T/n=n$.
	By a Chernoff bound,
	\begin{align*}
		\Pr\!\bigl(X<(9/10)n\bigr)\ \le\ \exp\!\Bigl(-\frac{(0.1)^2}{2}\,n\Bigr)\ =\ e^{-\Omega(n)}.
	\end{align*}
	Thus the process reaches $J$ within $T$ iterations with probability $1-e^{-\Omega(n)}$.
	
	\medskip\noindent
	\emph{(B) DPO before $J$ is $o(1)$.} We split iterations into two regimes by a constant
	$q\in(0,\tfrac{3}{16})$, say $q:=\tfrac{1}{12}$.
	
	\smallskip
	\underline{Regime B.1: $M\le qn$.} In a single iteration, to produce a DPO the hypermutation must
	flip every first-$L$ position that currently holds a $1$. This number is at least $\min\{L,\mathrm{LO}(x)\}\ge L$
	once $\mathrm{LO}(x)\ge L$, and at most $L$ in any case; hence, conditioning only on $M$,
	\begin{align*}
		\Pr(\text{DPO in this iteration}\mid M)\ \le\ \Bigl(\frac{M}{n}\Bigr)^{L}\ \le\ q^{L}\ =\ n^{-\beta(q)},
	\end{align*}
	where $\beta(q):=-\ln q>1.67$ for $q=\tfrac{1}{12}$.
	Next we bound how many \emph{Regime B.1} iterations can occur before either reaching $J$ or moving to
	\emph{Regime B.2}. While $M\le qn$, each \emph{accepted} equal-fitness tail flip increases
	$\mathrm{HD}(x,x^\top)$ by $1$ and hence increases $M$ by $1$ every $\lceil 1/c\rceil$ such accepts.
	To cross from $M\le qn$ to $M>qn$ requires at most $\lceil qn/c\rceil$ such accepts. The probability that
	an iteration yields an equal-fitness tail accept is at least a constant (tail length is $\ge 0.1n-1$ as
	long as $\mathrm{LO}(x)\le (9/10)n-1$), so by standard Chernoff bounds, with probability
	$1-e^{-\Omega(n)}$ the total number of \emph{Regime B.1} iterations is $O(n)$. Therefore,
	\begin{align*}
		\Pr(\text{DPO during Regime B.1})\ \le\ O(n)\cdot n^{-\beta(q)}\ =\ o(1).
	\end{align*}
	
	\smallskip
	\underline{Regime B.2: $M>qn$.} Now the hypermutation flips at least $qn$ distinct positions.
	Even if all first-$L$ positions are included (probability $\le 1$), to end in the DPO set we must
	also satisfy the tail-density constraint $|y|_1>\tfrac{3n}{4}$ after at least $qn-L$ tail flips.
	By a Chernoff bound for sampling without replacement (hypergeometric tails), the probability of
	ending with more than $\tfrac{3n}{4}$ ones in the tail after $qn-L$ flips is at most $e^{-\gamma n}$
	for some $\gamma(q)>0$ depending only on $q$. Hence, uniformly over states in \emph{Regime B.2},
	\[
	\Pr(\text{DPO in one iteration})\ \le\ e^{-\gamma n}\!,
	\]
	and, by a union bound over at most $T=n^2$ iterations, the total contribution of \emph{Regime B.2}
	is at most $n^2 e^{-\gamma n}=e^{-\Omega(n)}$.
	
	\smallskip
	Combining B.1 and B.2, we obtain
	\begin{align*}
		\Pr(\text{DPO before }J\text{ within }T)\ =\ o(1).
	\end{align*}
	
	\medskip\noindent
	\emph{(C) BJO before $J$ is exponentially unlikely.} Let $y^\star=0^{2k+1}1^{n-2k-1}$.
	In any iteration, flips are chosen uniformly without replacement. The final offspring equals $y^\star$
	\emph{if and only if} the set of flipped positions is exactly the difference set
	$D:=\{i\in[n]: x_i\neq y^\star_i\}$ (no re-flips are possible within an iteration). Therefore:
	\begin{align*}
		&\Pr(\text{offspring}=y^\star\text{ in a given iteration}\mid x)\ =\
		\begin{cases}
			\binom{n}{|D|}^{-1}, & \text{if } M(x)=|D|,\\
			0, & \text{if } M(x)\neq |D|.
		\end{cases}
	\end{align*}
	Since $M(x)\le cn$ for all $x$, we have the uniform bound
	\begin{align*}
		\Pr(\text{offspring}=y^\star\text{ in a given iteration})\
		\le\
		\max_{0\le m\le cn} \binom{n}{m}^{-1}
		\ \le\
		\binom{n}{\lfloor cn\rfloor}^{-1}
		\ \le\
		2^{-H(c)\,n+o(n)}
		\ =\
		e^{-\Omega(n)},
	\end{align*}
	where $H(\cdot)$ is the binary entropy in bits. This bound is independent of the parent, the reference,
	and the iteration index. A union bound over $T=n^2$ iterations gives
	$\Pr(\text{BJO before }J\text{ within }T)\le n^2 e^{-\Omega(n)}=e^{-\Omega(n)}$.
	
	\medskip\noindent
	\emph{(Conclusion).} Putting (A), (B), and (C) together,
	\begin{align*}
		\Pr(\mathcal{E}_{\mathrm{junction}})\ 
		&\ge\ 
		\Pr(\text{reach }J\text{ within }T)\ -\ \Pr(\text{DPO before }J)\ -\ \Pr(\text{BJO before }J)
		\\
		&\ge\ 
		\bigl(1-e^{-\Omega(n)}\bigr)\ -\ o(1)\ -\ e^{-\Omega(n)}
		\ =\ 1-o(1).
	\end{align*}
	All bounds hold uniformly over the arbitrary fixed reference $x^\top$, and the probability is taken
	over the randomness of the uniform initialization and the subsequent algorithmic randomness.
\end{proof}
