...
\begin{theorem}
	...
	\textcolor{purple}{\firstBest~with} \linHD\ (factor $c<\tfrac{1+4\varepsilon}{3-4\varepsilon}$) and ageing parameter $\tau=\Omega(n^{1+\epsilon'})$
...
For stage (ii) we will use the generalization of Markov's Inequality by Ville~\cite{Doob1953}(Ch.~VII) which will allows us to show that the mutation potential stays bounded with constant probability.
...
algorithm into a configuration with \textcolor{purple}{$n-d+2$ 1-bits i.e., ($r_0=2$) and the minimal Hamming distance of $HD(r_0)=2$ to the $best$ solution which had $n+d$ 1-bits.}  The following lemma formalises this.
...
envelope suitable for applying \textcolor{purple}{Ville's inequality for non-negative supermartingales (Theorem~\ref{thm:ville}).}
