We now present the proof sketch of our main result on the  $\mathrm{CLIFF}_d$ function, together with 
...
\begin{theorem}[Main Runtime Bound]
	Let $d < n(\tfrac14-\varepsilon)$ for fixed $\varepsilon>0$, and assume IPH 
	with linear mutation potential $M(x)=\max\{1,\lfloor c\,\mathrm{HD}(x,x^\top)\rfloor\}$ 
	for some 
	\[
	c < \frac{1+4\varepsilon}{3-4\varepsilon}.
	\]
	If the ageing threshold satisfies $\tau=\Omega(n^{1+\varepsilon'})$ for some 
	$\varepsilon'>0$, then the $(1+1)$~Opt-IA optimises $\mathrm{CLIFF}_d$ in expected
	\[
	O\!\left(\frac{n^{5}\log(n/d)}{d^{3}} + n^{2}\log d\right)
	\]
	fitness evaluations, assuming a pessimistic $\Theta(n)$ evaluation cost per 
	iteration.
\end{theorem}

To establish this result, we first analyse how the algorithm reaches a highly 
structured ``canonical'' state shortly after ageing at the cliff-top, and then 
analyse its behaviour on the second slope under this canonical start.
...
the iteration of ageing followed by two additional iterations can place the 
algorithm into a configuration with low Hamming distance to the locked 
reference and with mutation potential~$1$.  The following lemma formalises this.
...
linear \emph{buffer} capturing the invariant $c\,\mathrm{HD}(r)\le r$.
...
envelope suitable for applying Ville's inequality.

\begin{lemma}[Exponential Drift Envelope]
	Let $\rho=2d/n$.  
...
Ville's inequality for nonnegative supermartingales provides the standard tool 
for turning such a drift envelope into a bound on barrier crossing.

\begin{theorem}[Ville's Inequality]
	If $(M_t)$ is a nonnegative supermartingale, then for any $a>0$,
	\[
	\Pr\!\left(\sup_{t\ge0} M_t \ge a\right)\le \frac{\mathbb{E}[M_0]}{a}.
	\]
\end{theorem}
