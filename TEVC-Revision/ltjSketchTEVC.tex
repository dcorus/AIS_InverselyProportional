...
short slope, \new{called \rev{\emph{block-jump}} slope,} that \textcolor{black}{deterministically increases toward its unique local optimum; reaching 
...
plateau of globally optimal fitness, \new{which we call  \rev{\emph{deep-prefix plateau}}}.  
...
the plateau $\mathcal{D}_\rho$. \textcolor{black}{On the other hand, algorithms with lower mutation rates, such as those using SBM will with high probability not be able to flip the logarithmic number of bits required to reach the global optimum, thus have a runtime of $n^{\Omega(\log n)}$ with high probability.}
...
\begin{proof}
	Let $T:=n^2$. \textcolor{black}{We will first show that unless we sample a solution from $\mathcal{B}_k$ or $\mathcal{D}_\rho$ , we will sample a solution from $J$ in $T$ iteration with overwhelming probability.}
...
\textcolor{black}{A coupon-collector argument then implies} that the expected number of epochs needed to reach both the block-jump basin and the deep-prefix plateau is $O(1/p_B + 1/p_D)$.
