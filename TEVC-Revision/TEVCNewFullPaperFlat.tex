...
\begin{proof}
	Let $T:=n^2$. \textcolor{blue}{We will show that in $T$ iterations with high probability we sample a solution from  $J$ before we sample one from $\mathcal{B}_k$ or $\mathcal{D}_\rho$.}
...
\textcolor{blue}{The following two lemmata will condition throughout on the event that during an ageing epoch the junction $J$ is reached.
...
By Lemma~\ref{lem:junction-first-density-Bk} we reach the junction $J$ with probability \ \(1-o(1)\).
	Starting an epoch in the junction, the two lemmata give
