...
\begin{table*}[t!]
	\caption{Expected runtimes of the different mutation operators when embedded in a (1+1) algorithmic framework for the benchmark functions considered in this paper. For all results the optimum is unknown to the algorithms. \rev{The $*$ indicates that we do not find it worth effort to perform a formal analysis, yet for reasons provided at the end of Section~\ref{sec:ltj}, the upper bound is correct.} 
...
%%% FIX 2 — clarify f_OPT non-negativity and meaning
 The original inversely proportional hypermutation operator (\IPHfcm{}) proposed for Opt-IA uses mutation potential  $M=\lceil(1-f_{\text{OPT}}/v)\rceil cn$ for minimisation problems, where \textcolor{blue}{$f_{\text{OPT}}\ge 0$ is the optimal fitness value and $v>0$ is the fitness of the current individual,} 
...
the expected time to optimise \textsc{OneMax} is larger than \textcolor{blue}{$(1-2^{-\Omega(n)})\sum_{i=1}^{n/3} {\color{black}\left(\frac{n}{2i}-1\right)} \cdot i = \Omega(n^2)$ (the factor $(1-2^{-\Omega(n)})$ accounts for the w.o.p.\ Chernoff event; since $2^{-\Omega(n)}=o(1)$, the bound holds unconditionally)} \rev{since the operator cannot improve the \textsc{OneMax} function value by more than one in each application due to FCM}.
\end{proof}
...
\rev{We can now proceed with the analysis of the exponential-based potential.}
 	...
 	%             \STATE{ $M :=$ \expoHD};
 		...
 		%selected uniformly at random one after another until a \textit{constructive 
 			%mutation} happens;}
				...
				\textcolor{blue}{\IF{$f(w) \geq f(\text{best})$}
				\STATE{$\text{best}:=w$};
				\ENDIF} 
...
%%% FIX 12b — Algorithm 4 (FirstBest): add best-check after reinitialisation
 	\begin{algorithm}[t] 
 		...
 		\begin{algorithmic}[1] % The number tells where the line numbering should start
 			...
 			{\color{black}\hspace{0.3em} \IF{$f(w) = f(\text{best})$} 
 			\STATE \hspace{0.3em} $\text{best}:=w$
 			\ENDIF }
 			\STATE \hspace{0.3em} reinitialise $w$ uniformly at random with $\alpha(w):=0$
			\textcolor{blue}{\IF{$f(w) > f(\text{best})$}
			\STATE $\text{best}:=w$
			\ENDIF}
 			\ENDIF
 			\ENDFOR
