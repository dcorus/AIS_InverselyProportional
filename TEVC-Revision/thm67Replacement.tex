...
\new{both} algorithms become very efficient for hill-climbing on \textsc{OneMax}.

\begin{lemma}[RLS Reduction]\label{lem:RLS-reduction}
Let $f:\{0,1\}^n\to\mathbb{R}$ and let $T_{\mathrm{RLS}}$ denote the expected
runtime of RLS (1-bit flip, accept if $\geq$) on $f$.
Suppose that the \lastBest{} \emph{(}resp.\ \firstBest{} on functions admitting
no neutral moves\emph{)} with ageing parameter~$\tau$ satisfies:
\emph{(i)}~the mutation potential $M=1$ throughout every run in which ageing is
not triggered, and
\emph{(ii)}~the probability $p:=\Pr[A]$ that ageing is triggered before the
optimum is found satisfies $p=o(1)$ and $\tau\cdot p=o(1)$.
Then the expected runtime $T'$ of the algorithm satisfies
$\mathbf{E}[T'] = (1\pm o(1))\,T_{\mathrm{RLS}}$.
\end{lemma}

\begin{proof}
By~(i), conditional on $\lnot A$ the algorithm runs identically to RLS.
...
\textcolor{black}{In this section, we have proven the results for \linHD. Naturally the same results follow for the \expoHD~ operator as upper bounds which always has at most the same mutation rate or lower.}
