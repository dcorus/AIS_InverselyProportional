...
\begin{lemma}[Phase MGF, refined]\label{lem:phase-mgf}
	...
	Since the improving attempt must flip a zero at the first flip\textcolor{purple}{?}, we have $\Delta^{\mathrm{succ}}_z=1$\textcolor{purple}{?}. Hence
	\[
	Y_z\ :=\ S_z\textcolor{purple}{?}+(\Delta^{\mathrm{succ}}_z-1)\ =\ S_z\ \ge 0.
	\]
...
\begin{proof}
	For a single \emph{failed} attempt at zero-count $z$, let $\xi$ count the number of $1\!\to 0$ flips \textcolor{purple}{while below the parent} before the first catch-up.
	...
	\begin{align*}
		...
		\ \le\ \EE\!\big[\textcolor{purple}{t^{F}}\big].
	\end{align*}
