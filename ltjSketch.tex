...
\begin{lemma} \label{lem:junction-first-density-Bk}
	Starting from any random individual, the $(1+1)$~Opt-IA$_{\textsc{FirstBest}}$~using \IPHfcm{}~with \linHD~\textcolor{purple}{and $c???$}  reaches the junction $J = \{x : \mathrm{LO}(x)=0.9n-1\}$ before accepting any individual in $\mathcal{B}_k$ or $\mathcal{D}_\rho$ with high probability.
\end{lemma}
...
\begin{lemma} \label{lem:BJO-epoch}
	Let an ageing epoch begin at the junction  \textcolor{purple}{and let $c >??$}.
...
\textcolor{purple}{$M(x)= cn/20 \ge n/60$} throughout the epoch.  
