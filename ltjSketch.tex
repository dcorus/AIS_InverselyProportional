...
Two off-slope structures contain the \textcolor{purple}{global optima} but are inaccessible to 
standard hill-climbers.  
The set $\mathcal{B}_k$ begins with the prefix $0^{r}1^{(0.9n-r)} *$ and forms a 
short slope, \new{called block-jump slope,} that \textcolor{purple}{deterministically increases toward its unique optimum; reaching 
...
$0^{L} *$ \textcolor{purple}{whose suffix has ones-density at least $\rho$}, and forms a wide 
plateau of globally optimal fitness, \new{which we call  deep-prefix plateau}, and is not a slope.  
...
the plateau $\mathcal{D}_\rho$. \textcolor{purple}{On the other hand, algorithms with lower mutation rates, such as those using SBM...}
...
$M(x)\ge n/60$ \textcolor{purple}{(Why?)} throughout the epoch.  
