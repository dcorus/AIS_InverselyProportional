...
\begin{table*}[t!]
	\caption{Expected runtimes of the different mutation operators when embedded in a (1+1) algorithmic framework for the benchmark functions considered in this paper. For all results the optimum in unknown to the algorithms.}
	%The same result as $(1+1)~IA^{hype}$ using static hypermutation ($M=n$) has been shown for the original IPM of Opt-IA \cite{JansenZarges2011}.}
\begin{center}
	\resizebox{2\columnwidth}{!}{
...
in realistic applications where the optimum is unknown.
 ...
 \textcolor{purple}{Concerning the IPH operator for practical applications we  reintroduce the parameter $c\leq 1$ for determining the mutation potential when it grows linearly with the Hamming distance as in the original Opt-IA~\cite{JansenZarges2011}:}  
 \textcolor{purple}{
 $$M_\textsc{linHD} =\textcolor{blue}{\max}\left(\lfloor c \cdot H(x,\text{best}) \rfloor, 1 \right) $$
 }
\textcolor{purple}{The parameter will be useful to avoid jumping back to previously seen local optima when escaping them by accepting solutions of lower fitness. TO BE DISCUSSED: which will hinder exploration that is the main goal of the high mutation rates??.}
...
The expected runtime of the \new{$(1+1)$~Opt-IA$_{\textsc{FirstBest}}$ and the $(1+1)$~Opt-IA$_{\textsc{LastBest}}$} using \IPHfcm{}  %with \textcolor{green}{{\expoHD }} 
\textcolor{purple}{with {\linHD }} for optimising
\textsc{OneMax} is $\Theta(n\log n)$ with $\tau=\Omega(n^{1+\epsilon})$ {\color{blue}for any arbitrary} constant $\epsilon>0$.
\end{theorem}
\begin{proof}
Let $x_t$ be the current solution at the beginning of the iteration $t$. Immediately after the initialisation, the best seen solution is the current individual $x_1$ itself and the mutation potential is %$M=$ {\expoHD }$=n^{H(x_1,x_1)/n}=n^0=1$. \textcolor{blue}{Similarly the linear mutation is also 
...
\begin{theorem} \label{thm:leadingonesexpo}
The expected runtime of the \oneoneOPTIA~using \IPHfcm{}  %with {\expoHD } 
\textcolor{purple}{with {\linHD }} for optimising
...
Due to the high mutation rates of static hypermutations, even if they were to escape the local optima of \textsc{Cliff} (which is \new{very unlikely} due to FCM and linear mutation potentials), they would still jump back with high probability.  
