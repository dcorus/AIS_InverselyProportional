...
\textcolor{purple}{However, {\expoHD } only allows to flip at most $\sqrt{n}$ bits when far away from the optimum, which means that apart from at initialisation the algorithm hardly ever hypermutates.}
	...
	\textcolor{purple}{We do not update the current Best with individuals of equal fitness to the best because if trapped on a plateau of local optima, this allows to increase the hamming distance from the local optimum, thus increases the chances of escaping via higher mutation rates. We, instead, update the Best to the last seen individual of highest fitness (i.e. the current solution) when ageing is triggered because if a local optima has been escaped via a small mutation, this limits the hamming distance between the new solution and the Best, thus decreases the chances of jumping back to the previous local optimum.}
	\textcolor{green}{While this new update mechanism allows the algorithm to escape the local optimum of CLIFF, this behaviour is closely tied to the idiosyncrasies of that landscape. In particular the fact that even moderately large mutations yield an offspring in the basin of attraction of the local optimum while any efficient algorithm must follow the second gradient towards the global optimum. Thus the reason why small mutations near the previous best seen are beneficial in CLIFF is because we want to avoid going near the local optima over and over again rather than investing in a thorough exploitation in its vicinity, which is the intuitive reasoning behind IPH. As a solution in the second gradient goes higher and higher in fitness levels it can tolerate larger and largers mutation without going back to the first slope.  }
	
	
	In this section we consider the usage of  {\expoHD } in realistic applications where the optimum is unknown.
...
The expected runtime of the \oneoneOPTIA~using \IPHfcm{}  with \textcolor{green}{{\expoHD }} \textcolor{purple}{and with {\linHD }} for optimising
...
\begin{theorem} \label{thm:leadingonesexpo}
	The expected runtime of \textcolor{green}{the old  \oneoneOPTIA~} using \IPHfcm{}  with {\expoHD } \textcolor{purple}{and with {\linHD }} for optimising
